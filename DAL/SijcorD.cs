...
namespace DAL
{
    public class SijcorD
    {
        ...
        public static List<Entities.Sijcor> GetSijcor(int anio, int cod_tipo_liq, int nro_liquidacion, string periodo, int opcion)
        {
            ...
            strSQL = @"SELECT e.legajo, cuil = REPLACE(e.cuil, '-', ''), 
                         e.nombre, e.tarea, categoria = convert(varchar(2), c1.cod_categoria), 
                         e.cod_regimen_empleado, 
                         cod_modalidad_contratacion =
                         CASE
                          WHEN e.cod_clasif_per = 1 THEN '0003'
                          WHEN e.cod_clasif_per = 2 THEN '0004'
                          ELSE '0004'
                         END, 
                         cant_dias_trabajados = (SELECT isnull(SUM(unidades), 0)
                          FROM DET_LIQ_X_EMPLEADO dlxe2
                          WHERE dlxe2.anio = dlxe.anio AND
                          dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                          dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                          dlxe2.legajo = dlxe.legajo AND
                          dlxe2.cod_concepto_liq BETWEEN 300 AND 310), 
                         sueldo390 = (SELECT isnull(SUM(dlxe2.importe), 0)
                         FROM DET_LIQ_X_EMPLEADO dlxe2
                         WHERE dlxe2.anio = dlxe.anio AND
                         dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                         dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                         dlxe2.legajo = dlxe.legajo AND
                         dlxe2.cod_concepto_liq in (390)), 
                         sueldo390BIS = (SELECT isnull(SUM(dlxe2.importe), 0)
                         FROM DET_LIQ_X_EMPLEADO dlxe2
                         WHERE dlxe2.anio = dlxe.anio AND
                         dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                         dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                         dlxe2.legajo = dlxe.legajo AND
                         dlxe2.cod_concepto_liq in (390, 499)), 
                         otroscodigos = (SELECT isnull(SUM(dlxe2.importe), 0)
                         FROM DET_LIQ_X_EMPLEADO dlxe2
                         WHERE dlxe2.anio = dlxe.anio AND
                         dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                         dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                         dlxe2.legajo = dlxe.legajo AND
                         dlxe2.cod_concepto_liq in (12, 154)), 
                         conceptos_no_remunerativos_1 = (SELECT isnull(SUM(dlxe2.importe), 0)
                         FROM DET_LIQ_X_EMPLEADO dlxe2
                         JOIN CONCEPTOS_LIQUIDACION B on
                         B.cod_concepto_liq <> 390 AND
                         B.cod_concepto_liq = dlxe2.cod_concepto_liq AND
                         B.suma <> 0 AND B.sujeto_a_desc = 0
                         WHERE dlxe2.anio = dlxe.anio AND
                         dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                         dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                         dlxe2.legajo = dlxe.legajo and dlxe2.cod_concepto_liq not in 
                         (410, 420, 421, 430, 440, 450, 470, 480, 481, 482, 496, 500, 510, 520, 540, 570, 580, 590)),
                         aguinaldo = (SELECT isnull(SUM(dlxe2.importe), 0)
                         FROM DET_LIQ_X_EMPLEADO dlxe2
                         JOIN CONCEPTOS_LIQUIDACION B on
                         B.cod_concepto_liq <> 390 AND
                         B.cod_concepto_liq = dlxe2.cod_concepto_liq AND
                         ...
                         dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                         dlxe2.legajo = dlxe.legajo AND
                         dlxe2.cod_concepto_liq between 16 and 19), 
                         valor_codigo_800 = (SELECT isnull(sum(valor_concepto_liq), 0)
                         FROM CONCEP_LIQUID_X_EMPLEADO clxe
                         Where clxe.legajo = dlxe.legajo And clxe.cod_concepto_liq = 800),  
                          importe_adherente_voluntario = (SELECT isnull(SUM(dlxe2.importe), 0)
                          FROM DET_LIQ_X_EMPLEADO dlxe2
                          WHERE dlxe2.anio = dlxe.anio AND
                          dlxe2.cod_tipo_liq = dlxe.cod_tipo_liq AND
                          dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                          dlxe2.legajo = dlxe.legajo AND
                          dlxe2.cod_concepto_liq = 800), 
                         valor_codigo_624 = (SELECT isnull(sum(valor_concepto_liq), 0)
                         FROM CONCEP_LIQUID_X_EMPLEADO clxe
                         Where clxe.legajo = dlxe.legajo And clxe.cod_concepto_liq = 624)";
            if (opcion == 1)
            {
                strWhere = @"FROM LIQ_X_EMPLEADO dlxe 
                             JOIN EMPLEADOS e ON 
                             e.legajo = dlxe.legajo 
                             JOIN CATEGORIAS c1 on 
                             e.cod_categoria = c1.cod_categoria 
                             LEFT JOIN CARGOS c ON 
                             c.cod_cargo = e.cod_cargo 
                             WHERE 
                             dlxe.anio = @vAnio
                             AND dlxe.cod_tipo_liq = @vCod_tipo_liq
                             AND dlxe.nro_liquidacion =@vNro_liquidacion
                             ORDER BY dlxe.legajo, dlxe.nro_liquidacion";
            }
            else
            {
                strWhere = @"FROM LIQUIDACIONES L
                            JOIN LIQ_X_EMPLEADO dlxe ON
                            L.anio = dlxe.anio AND
                            L.cod_tipo_liq = dlxe.cod_tipo_liq AND
                            L.nro_liquidacion = dlxe.nro_liquidacion
                            JOIN EMPLEADOS e ON
                            e.legajo = dlxe.legajo
                            JOIN CATEGORIAS c1 on
                            e.cod_categoria = c1.cod_categoria
                            ...
                            WHERE L.prueba = 0 
                            AND L.anio = @vAnio
                            AND L.periodo = @vPeriodo
                            ORDER BY dlxe.legajo, l.aguinaldo";
                //dlxe.nro_liquidacion
            };
                ...
                if (dr.HasRows)
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        if (!dr.IsDBNull(sueldo390))
                        {
                            ...
                            if (sueldo >= 999999)
                                eSijcor.sueldo = string.Format("{0:000000.0}", sueldo).Replace(".", ",");
                            else
                                eSijcor.sueldo = string.Format("{0:000000.00}", sueldo).Replace(".", ",");
                        }
                        else
                        ...
                        if (!dr.IsDBNull(conceptos_no_remunerativos_1))
                        {
                            ...
                            decimal valor = dr.GetDecimal(conceptos_no_remunerativos_1);
                            if (valor < 0)
                            {
                            ...
                            {
                                //int codigo = Convert.ToInt32(eSijcor.categoria);
                                //if (codigo > 24)
                                eSijcor.conceptos_no_remunerativos = string.Format("{0:000000.00}", dr.GetDecimal(conceptos_no_remunerativos_1)).Replace(".", ",");
                                //else
                                //    eSijcor.conceptos_no_remunerativos = "000000,00";
                        ...
                        {
                            decimal valor = dr.GetDecimal(aguinaldo);
                            if (valor > 999999)
                                //eSijcor.retroactividades = "000000,0";
                                eSijcor.aguinaldo = string.Format("{0:000000.0}", dr.GetDecimal(aguinaldo)).Replace(".", ",");
                            else
        ...
        public static List<Entities.Sijcor> GetSijcorConAguilucho(int anio, int cod_tipo_liq, int nro_liquidacion, string periodo, int opcion)
        {
            ...
            strSQL = @"select e.legajo, cuil = REPLACE(e.cuil, '-', ''), 
                        ...
                        dlxe2.nro_liquidacion = dlxe.nro_liquidacion AND
                        dlxe2.legajo = dlxe.legajo and dlxe2.cod_concepto_liq not in 
                        (410, 420, 421, 430, 440, 450, 470, 
                         480, 482, 496, 500, 510, 520, 540, 570, 580, 590)),
                        aguinaldo = (SELECT isnull(SUM(dlxe2.importe), 0)
                        FROM DET_LIQ_X_EMPLEADO dlxe2
                        JOIN CONCEPTOS_LIQUIDACION B on
                        B.cod_concepto_liq <> 390 AND
                    ...
                    while (dr.Read())
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                //int codigo = Convert.ToInt32(eSijcor.categoria);
                                //if (codigo > 24)
                                //    eSijcor.conceptos_no_remunerativos = string.Format("{0:000000.00}", monto_no_remunerativo).Replace(".", ",");
                                //else
                                //    eSijcor.conceptos_no_remunerativos = "000000,00";

                                if (monto_no_remunerativo > 999999)
                                    eSijcor.conceptos_no_remunerativos = string.Format("{0:000000.0}", monto_no_remunerativo).Replace(".", ",");
                                else
                                    eSijcor.conceptos_no_remunerativos = string.Format("{0:000000.00}", monto_no_remunerativo).Replace(".", ",");
                            }
                            ////eSijcor.conceptos_no_remunerativos = "000000.00";
                            ...
                            if (monto_aguinaldo > 999999)
                                //eSijcor.retroactividades = "000000,00";
                                eSijcor.aguinaldo = string.Format("{0:000000.0}", monto_aguinaldo).Replace(".", ",");
                            else
                    ...
                    while (dr.Read())
                    {
                        ...
                        if (!dr.IsDBNull(sueldo390))
                        {
                            ...
                            if (sueldo >= 999999)
                                ...
                                eSijcor.sueldo = string.Format("{0:00000.00}", sueldo).Replace(".", ",");
                        }
                        else
                            eSijcor.sueldo = "000000,00";
                        ...
                        if (!dr.IsDBNull(conceptos_no_remunerativos_1))
                        {
                            eSijcor.conceptos_no_remunerativos =
                                string.Format("{0:000000.00}", dr.GetDecimal(conceptos_no_remunerativos_1)).Replace(".", ",");
                            //2019
                            //dr.GetDecimal(conceptos_no_remunerativos_1) + dr.GetDecimal(conceptos_no_remunerativos_2)).Replace(".", ",");
                            //dr.GetDecimal(conceptos_no_remunerativos_1) + dr.GetDecimal(conceptos_no_remunerativos_497) +
                            //dr.GetDecimal(conceptos_no_remunerativos_2) + dr.GetDecimal(conceptos_no_remunerativos_3)).Replace(".", ",");
                            decimal monto_no_remunerativo = dr.GetDecimal(conceptos_no_remunerativos_1);
                            if (monto_no_remunerativo < 0)
                            {
                                eSijcor.conceptos_no_remunerativos = "000000,00";
                            }
                        ...
                        if (!dr.IsDBNull(aguinaldo))
                        {
                            decimal importe_aguinaldo = dr.GetDecimal(aguinaldo);
                            if (importe_aguinaldo > 1000000)
                                eSijcor.retroactividades = "000000,0";
                            //eSijcor.aguinaldo = string.Format("{0:000000.0}", dr.GetDecimal(aguinaldo)).Replace(".", ",");
                            //else
                            eSijcor.aguinaldo = string.Format("{0:000000.00}", dr.GetDecimal(aguinaldo)).Replace(".", ",");
