...
var aHeight = $(document).height(); var aWidth = $(document).width(); $('body').append('<div class="appriseOverlay" id="aOverlay"></div>'); $('.appriseOverlay').css('height', aHeight).css('width', aWidth).fadeIn(100); $('body').append('<div class="appriseOuterRed"></div>'); $('.appriseOuterRed').append('<div class="appriseInner"></div>'); $('.appriseInner').append(string); $('.appriseOuterRed').css("left", ($(window).width() - $('.appriseOuterRed').width()) / 2 + $(window).scrollLeft() + "px"); if (args)
{if(args['animate'])
{var aniSpeed=args['animate'];if(isNaN(aniSpeed)){aniSpeed=400;}
$('.appriseOuterRed').css('top', '-200px').show().animate({ top: "100px" }, aniSpeed);
}
else
{ $('.appriseOuterRed').css('top', '100px').fadeIn(200); } 
}
else
{ $('.appriseOuterRed').css('top', '100px').fadeIn(200); }
if(args)
...
$('.appriseOverlay').remove(); $('.appriseOuterRed').remove(); if (callback)
